...
interface QuoteDetailModalProps {
  ...
  onStatusUpdate: (id: string | number, newStatus: QuoteRequest['status']) => void;
}
        ...
        <div className="p-8 sm:p-10 border-b border-gray-50 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${quote.status === 'pending' ? 'bg-orange-50 text-orange-600 border border-orange-100' :
          ...
          <div className="flex px-10 border-b border-gray-100 overflow-x-auto gap-8">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-3 py-6 border-b-2 transition-all cursor-pointer whitespace-nowrap ${activeTab === tab.id
            ...
            {activeTab === 'customer' && (
              <div className="space-y-8 animate-in fade-in duration-300">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <InfoItem label="Müşteri Tipi" value={quote.customer_id ? `Kayıtlı Müşteri` : 'Kayıtlı Olmayan Kullanıcı'} />
                  <InfoItem label="Firma Adı" value={quote.customer_company_name || quote.guest_company_name || '---'} />
                  <InfoItem label="E-posta" value={quote.customer_email || quote.guest_email || '---'} />
                  <InfoItem label="Talep Tarihi" value={formatDate(quote.created_at)} />
                </div>
                {quote.customer_id && (
